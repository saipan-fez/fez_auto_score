...
## 制限事項
* 特定の環境で一部の情報(国名・攻守・職業・スキル)が取得できません
* フルスクリーン設定の環境では動作しません(エラーメッセ―ジが表示されます)
...
#### 4.1 OBSでの設定方法
1. ソースに「テキスト(GDI+)」を追加する
2. 「ファイルからの読み取り」にチェックを入れる
3. 「参照」ボタンを押下し、`ドキュメントフォルダ\fez_auto_score\LatestScore.txt`を選択する  

> OBS以外の配信ソフトをお使いの場合は「配信ソフト名+テキストファイル」などで検索して設定してください。  
> なお、テキストの書式や出力するスコアは「設定」から変更できます。

### 5. 同時起動ショートカットの作成
FEZ起動時に当ツールも併せて起動するショートカットをデスクトップに作成します。  
デフォルトのインストール先以外にインストールされている場合は、  
ダイアログに従い`FEzero.exe`のインストール先を手動で指定してください。
...
## FAQ
### フルスクリーン設定で動くようになりませんか？
技術的には可能ですが、FEZ本体のプログラム内容を書き換えるような処理となり、  
チートツールとの誤認識される恐れがあるため今後も対応しません。
