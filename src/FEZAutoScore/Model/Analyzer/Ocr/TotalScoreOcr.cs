...
{
    public class TotalScoreOcr : IntegerOcr
    {
        ...
        protected override unsafe string Process(Bitmap bitmap)
        {
            ...
            try
            {
                ...
                for (byte* ptr = startPtr; ptr < endPtr; ptr += bpp)
                {
                    ...
                    var r4c = ToColor(ptr + 4 * bpp); // 4px横のピクセルの色
                    var r5c = ToColor(ptr + 5 * bpp); // 5px横のピクセルの色
                    ...
                    else if (r1c == White && r2c == White && r3c != White)
                    {
                        // Group:3px
                        // 指定のピクセルの色をチェック
                        var c1 = ToColor(ptr + stride * 4 - 1 * bpp); // (-1, +4)px
                        var c2 = ToColor(ptr + stride * 5 - 1 * bpp); // (-1, +5)px
                        var c3 = ToColor(ptr + stride * 6 - 1 * bpp); // (-1, +6)px
                        var c4 = ToColor(ptr + stride * 5 - 0 * bpp); // ( 0, +5)px
